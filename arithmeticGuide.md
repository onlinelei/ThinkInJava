...
<img src="img/20200618175807.png" alt="An example of a red-black tree" style="zoom:67%;" />
...
![image-20220630164602284](img/image-20220630164602284.png)
...
![image-20220630163646144](img/image-20220630163646144.png)





![image-20220630164323997](img/image-20220630164323997.png)
...
![image-20220630175303162](img/image-20220630175303162.png)



判断有环连表

![image-20220630201508516](img/image-20220630201508516.png)
...
1. 依次比较数组中相邻的两个元素的大小若a[j] > a[j+1] 则交换两个元素的位置，一轮结束以后，最大的元素会移至末尾。
2. 重复以上步骤，直至数组有序。

优化方式：

1. 计算下标位置通过 L + (R-L)/2 或者 R+L >>> 1来计算，避免出现负数情况
2. 如果没有再发生交换，退出最外层循环
3. 记录最后一次交换的下标， 作为下次循环的的终点位置，简化循环次数

## 2. 插入排序

1. 将数组分为两个区域，排序区和未排序区，每一轮从未排序区选择一个元素插入到排序区（自己保证顺序）
2. 重复以上步骤，直至数组有序。
...
1. 将数组通下标公式分成等份数组
2. 每个数组进行插入排序，直至所有分组排序完成
3. 将整个数组进行一次插入排序

## 4. 快速排序

1. 每一轮快速排序选择一个基准点，进行分区。
2. 让小于基准点的数据进入一个分区，大于基准点的数据进入另一个分区。
3. 当分区完成后，基准点的位置就是它最终的位置。
4. 在子分区里重复这个步骤，直至分区元素个数小于或等于1，这体现分而治之的思想（divide-and-conquer）。

优化方式：

1. 随机选着基准点
2. 可以用单边循环，双边循环可以来加速排序
3. 时间复杂度O(nlogn) 最坏的情况O(n2)
4. 数据量大，效率明显，属于不稳定排序 
